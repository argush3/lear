...
# def get_lear_db_session(config):
...
# def get_unprocessed_firms(config):
    logger = prefect.get_run_logger()
    # db_engine = colin_init(config)
...
# def get_event_filing_data(config, unprocessed_firm_dict: dict):
    logger = prefect.get_run_logger()
    # colin_db_engine = colin_init(config)
...
# def clean_event_filing_data(config, event_filing_data_dict: dict):
    logger = prefect.get_run_logger()
    # colin_db_engine = colin_init(config)
    status_service = ProcessingStatusService(config.DATA_LOAD_ENV, colin_db_engine)
...
# def transform_event_filing_data(config, event_filing_data_dict: dict):
    logger = prefect.get_run_logger()
    # colin_db_engine = colin_init(config)
    status_service = ProcessingStatusService(config.DATA_LOAD_ENV, colin_db_engine)
    corp_num = event_filing_data_dict['corp_num']
...
def load_event_filing_data(config, app: any, colin_db_engine: engine, db, event_filing_data_dict: dict):
# def load_event_filing_data(config, colin_db_engine: engine, event_filing_data_dict: dict):
# def load_event_filing_data(config, event_filing_data_dict: dict):
    logger = prefect.get_run_logger()
    # colin_db_engine = colin_init(config)
    # app, db_lear = lear_init(config)

    # db.init_app()
    # app = Flask('init_lear')
    # app.config.from_object(config)
    # init_app(app)
    # db_lear_session = get_lear_db_session(config)
    # db.session = db_lear
    # db.init_app(app)
    # app.session = db_lear_session
        ...
        try:
            ...
            for idx, event_filing_data in enumerate(event_filing_data_arr):
                ...
                if not event_filing_data['is_in_lear'] and not event_filing_data['skip_filing']:
                    ...
                    # populate_filing_json_from_lear(db_lear_session, event_filing_data, business)
                    ...
                    # business = process_filing(config, filing.id, event_filing_data_dict, filing_data, db_lear_session)
                    business = process_filing(config, filing.id, event_filing_data_dict, filing_data, db)
...
def migrate_flow():
    ...
    # unprocessed_firms = get_unprocessed_firms(config)
    ...
    event_filing_data = get_event_filing_data.map(unmapped(config),
                                                  ...
                                                  # unprocessed_firm_dict=allow_failure(unprocessed_firms))
    ...
    cleaned_event_filing_data = clean_event_filing_data.map(unmapped(config),
                                                            ...
                                                            # allow_failure(event_filing_data))
                                                            event_filing_data)
    ...
    transformed_event_filing_data = transform_event_filing_data.map(unmapped(config),
                                                                    # unmapped(FLASK_APP),
                                                                    unmapped(db_colin_engine),
                                                                    # allow_failure(cleaned_event_filing_data))
    ...
    loaded_event_filing_data = load_event_filing_data.map(unmapped(config),
                                                          ...
                                                          unmapped(db_colin_engine),
                                                          unmapped(db_lear),
                                                          # allow_failure(transformed_event_filing_data))
